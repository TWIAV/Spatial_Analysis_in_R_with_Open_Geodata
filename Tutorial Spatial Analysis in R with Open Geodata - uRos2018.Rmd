...
The USA Rail Road hosted feature service only has one layer with called *USA Railroads*. 
...
The response also other valuable like the maximum number of records returned per request:
```{r}
...
`r hits` is way more that the maximum record count per request of `r maxRecordCount`!    
...
What is the number of records in the railroads dataframe? One would expect, no more than 1,000.
...
A lot of the railroads are labelled 'abandonned'. Of course you can do this *after* you fetched the records, but this isn't very efficient. Let's change the parameter value of `where` in our request, so only exploited rail roads are included in the response. 
