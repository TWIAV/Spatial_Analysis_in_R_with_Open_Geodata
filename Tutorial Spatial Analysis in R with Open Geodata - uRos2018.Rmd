...
NL_Regional_Statistics2017[cols] <- sapply(NL_Regional_Statistics2017[cols], 
                                           function(x){trimws(x) %>% as.factor()})
```
...
```{r}
NL_Provinces2017_data %>% select(Code, Name, Total_population, Region_code, Region_name) %>% as.data.frame()
```
...
```{r}
NL_Provinces2017_data$Name <- str_replace(NL_Provinces2017_data$Name," \\(PV\\)", "") %>% as.factor()
```
...
NL_Regions2017_data$Name <- str_replace(NL_Regions2017_data$Name," \\(LD\\)", "") %>% as.factor()
```
...
```{r}
pct <- NL_Regions2017_data$Total_population / sum(NL_Regions2017_data$Total_population) * 100 %>% round(1) %>% paste0("%")
