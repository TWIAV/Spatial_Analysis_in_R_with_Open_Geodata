...
**Pro tip**: use `purl` to extract all the R code from the manual
...
A request to this URL will return the capabilities of service:
...
A hosted feature service can have multiple layers. Every layer has a name and an id. The id is necessary for constructing the appropriate URL to fetch the features.    
...
But wait, all the railroads in the United States of America, wouldn't that be *a lot* of records? Let's find out, before we fetch them all.    
We can do this by specifying the query parameter `returnCountOnly = "true"`:    
...
`r hits` is way more than the maximum record count per request of `r maxRecordCount`!    
...
What is the number of records in the railroads dataframe? One would expect, not more than `r maxRecordCount`.

```{r}
nrow(railroads)
```

Isn't that strange? The dataframe contains all `r hits` records. How is this possible?    
...
A lot of the railroads are labelled 'abandonned'. Of course we can limit the selection *after* we fetched the records, but this isn't very efficient. Let's change the parameter value of `where` in our request, so only railroads in use are included in the response. 
...
`r nrow(exploited_railroads)` is still a lot of records, but let's have a look at the result and add the railroads on top of a map of the USA:
