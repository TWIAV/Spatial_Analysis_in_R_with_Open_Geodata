...
```{r}
NL_Airports <- rbind(NL_Airports, airport) # Oh yeah, of course: this will generate an error...
```
...
total_no_entries <- nrow(cbs_get_toc())
```


The table of contents can be retrieved with the fucnction `cbs_get_toc()`. As we browse through the table of content we do see a total of `r total_no_entries` entries. For most datasets all the metadata is in Dutch, but for some of them this information is also available in English.
...
total_no_entries_en <- nrow(toc_en)
```

When we split the TOC by language we do see that currently `r total_no_entries_en` datasets are available with metadata in English. These 'English' datasets are actually copies of their Dutch equivalents. (Maybe overtime all tables will be available with a description in English?)
...
```{r}
names(NL_Regional_Statistics2017) <- c("Code","Name","Year","Year_label","Total_population","Region_code","Region_name","Province_code","Province_name")
```
...
```{r}
NL_Regional_Statistics2017$Code <- as.factor(trimws(NL_Regional_Statistics2017$Code))
...
Now that we have prepared our data, we can extract the data about NUTS 1 and NUTs 2 respectively. At NUTS 1 level the Neherlands is divided into 4 regions ('Landsdelen') and at NUTS 2 level into 12 provinces:

```{r}

NL_Regions2017_data <- NL_Regional_Statistics2017 %>% filter(Code %like% "LD")
NL_Provinces2017_data <- NL_Regional_Statistics2017 %>% filter(Code %like% "PV")
```
...
```{r}
as.data.frame(NL_Provinces2017_data[,c(1,2,5,6,7)])
```
...
```{r}
NL_Provinces2017_data$Name <- as.factor(str_replace(NL_Provinces2017_data$Name," \\(PV\\)", ""))
```

Likewise, the region names in the column `Name` all have the suffix *(LD)*. We will also remove that:

````{r}
as.data.frame(NL_Regions2017_data[,c(1,2,5)])
NL_Regions2017_data$Name <- as.factor(str_replace(NL_Regions2017_data$Name," \\(LD\\)", ""))
```
...
barplot(NL_Provinces2017_data$Total_population) # Try this yourself - result not printed in this manual
```
...
```{r exbplot2}
barplot(NL_Provinces2017_data$Total_population/1000000, names = NL_Provinces2017_data$Name, las = 2, cex.axis = .6, cex.names = .6, ylab = "Inhabitants (* 1.000.000)", cex.lab = .8, space = 0, col = "lightblue", main = "Number of Inhabitants by Province - The Netherlands - 2017", cex.main = .8)
```
...
```{r}
barplot(NL_Provinces2017_data$Total_population/1000000, names = NL_Provinces2017_data$Name, las = 2, cex.axis = .6, cex.names = .6, ylab = "Inhabitants ( * 1,000,000)", col = NL_Provinces2017_data$Region_code, border = "grey", main = "Number of Inhabitants by Province - The Netherlands - 2017", cex.main = .8, legend.text = unique(NL_Provinces2017_data$Region_name), args.legend = list(x = 'topleft', bty = 'n', fill = unique(NL_Provinces2017_data$Region_name), border = 'grey'))
```
...
```{r}
pct <- round(NL_Regions2017_data$Total_population/sum(NL_Regions2017_data$Total_population)*100, 1)
pct <- paste0(pct, "%")
...
```{r}
NL_Municipalities2017_data <- NL_Regional_Statistics2017 %>% filter(Code %like% "GM")
nrow(NL_Municipalities2017_data)
```
Are there 713 municipalities in the Netherlands? No, not anymore!
...
```{r}
NL_Municipalities2017_data <- NL_Municipalities2017_data %>% filter(Total_population != "")
nrow(NL_Municipalities2017_data)
```
...
Now we want to plot a map with the regional subdivision. To be abel to do so we have to merge our data frame with a dataset containing geometry.
...
```{r}
NL_Municipalities2017 <- NL_Municipalities2017 %>% select(statcode,statnaam)
```

Please note: in the statement above we only select 2 columns, but the reulting object does also contain the `geometry`column. This is due to the *sticky* character of this `geometry` column. When you select a subset of columns, the geometry will always be included, unless you explicitly drop it. In other words: a subset of an `sf` object will in itself also be an `sf` object.


Now we will rename the columns:

```{r}
names(NL_Municipalities2017)[1:2] <- c("Code","Name")
...
```{r}
NL_Municipalities2017 <- merge(NL_Municipalities2017,NL_Municipalities2017_data, by = "Code")
...
qtm(shp = NL_Municipalities2017,
    ...
    fill.palette = palette(c("royalblue3","firebrick3","darkolivegreen4","goldenrod1")),
    borders = "grey",
    format = "NLD_wide")
...
We will start this exercise by dividin our municipalities in 3 groups: small, medium and large.
...
```{r ordered_factor}
NL_Municipalities2017$Category[NL_Municipalities2017$Total_population < 50000] <- "Small population"
NL_Municipalities2017$Category[NL_Municipalities2017$Total_population >= 50000 & NL_Municipalities2017$Total_population < 200000] <- "Medium population"
NL_Municipalities2017$Category[NL_Municipalities2017$Total_population >= 200000] <- "Large population"
```
...
```{r}
NL_Municipalities2017$Category <- factor(NL_Municipalities2017$Category, levels = c("Small population","Medium population","Large population"), ordered = TRUE)

class(NL_Municipalities2017$Category)
```
```{r change_categories, echo = FALSE, results = 'hide', message = FALSE}
NL_Municipalities2017$Category <- as.character(NL_Municipalities2017$Category)
NL_Municipalities2017$Category[NL_Municipalities2017$Total_population < 20000] <- "Very small population"
NL_Municipalities2017$Category <- factor(NL_Municipalities2017$Category, levels = c("Very small population","Small population","Medium population","Large population"), ordered = TRUE)
```
...
Geographic coordinate systems identify locations on the Earth's surface using longitude and latitude. Longitude is the distance East or West in angular distance, i.e. measured in degrees, from the Prime Meridian plane. Latitude is this distance North or South of the equatorial plane. And no, there is not just one 'Lat\\Lon coordinate system'. All geographic systems have the use the same reference, i.e. the Prime Meridian and the Equator, but the exact location on the Earth depends on several parameters like the ellipsoid and the datum used. So even if someone gives you data 'in degrees' you still have to find out which CRS to use. With a wrong CRS your data might end up centimeters or even several meters from the correct location. An important advantage of these geographic systems is their global reach - they can be used everywhere and are useful when dealing with international datasets. An important disadvantge is: distances cannot be easily measured in for example meters or kilomters in geographic CRSs.

Projected coordinate system have a much smaller reach, as these are based on Cartesian coordinates on an implicitly flat surface. They are only valid for a specific area - a country or even just a part of a country - where they have an origin, x and y axes, and a linear unit of measurement such as meters. We have seen several examples of data in such local, projected systems, e.g. EPSG:31287 for Austria, EPSG:3067 for Finland and EPSG:28992 for the Netherlands (but only onshore). These systems are ideal for dealing with data at a nationnal or subnational level. In such a case you shuld really stick with the local CRS. 
