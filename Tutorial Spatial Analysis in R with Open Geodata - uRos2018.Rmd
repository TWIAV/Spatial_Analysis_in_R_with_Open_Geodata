...
# Analysing spatial datasets {chapAnalysis}

The municipality of The Hague publishes some nice datasets on http://denhaag.dataplatform.nl. We'll use some of them to learn more about spatial en geometric operations:
...
bat_flight_paths <- st_read(url) %>% select(FUNCTION = FUNCTIE)
...
bat_flight_paths$FUNCTION <- plyr::mapvalues(bat_flight_paths$FUNCTION, from = levels_dutch, to = levels_eng)
``` 

## Length, area and distance
Lets begin with a simple question: What is the largest district in The Hague?
...
```{r}
districts %>% filter(area == max(area)) %>% select(name, area) %>% mutate(area = area / 10000) %>% st_set_geometry(NULL)
```
...
However, in this case you need to change the units (and no need to subdivide by 10,000):

```{r}
districts %>% filter(area == max(area)) %>% select(name, area) %>% mutate(area = units::set_units(area, ha)) %>% st_set_geometry(NULL)
```
That's better!

Next question: What are the lengths in kilometers - with two numbers after the decimal point - of the bat flight routes? 

```{r}
bat_flight_paths$length <- st_length(bat_flight_paths)
bat_flight_paths %>% mutate(length = round(units::set_units(length, km), 2)) %>% st_set_geometry(NULL)
```

How many meters are the trees with id 112964 and 3114766 apart from each other?
```{r}
