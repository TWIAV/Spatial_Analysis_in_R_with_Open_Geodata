...
NL_Regions2017_data <- NL_Regional_Statistics2017 %>% filter(Regional_level %like% "LD")
NL_Provinces2017_data <- NL_Regional_Statistics2017 %>% filter(Regional_level %like% "PV")
...
We have noticed that the province names in the column `Regional_level_label` all have the suffix *(PV)*. Let's get rid of it (but let's make sure it remains a factor):

```{r}
NL_Provinces2017_data$Regional_level_label <- as.factor(str_replace(NL_Provinces2017_data$Regional_level_label," \\(PV\\)", ""))
```
...
````{r}
NL_Regions2017_data$Regional_level_label <- as.factor(str_replace(NL_Regions2017_data$Regional_level_label," \\(LD\\)", ""))
NL_Regions2017_data
```
...
```{r exbplot2}
barplot(NL_Provinces2017_data$Total_population/1000000, names = NL_Provinces2017_data$Regional_level_label, las = 2, cex.axis = .6, cex.names = .6, ylab = "Inhabitants (* 1.000.000)", cex.lab = .8, space = 0, col = "lightblue", main = "Number of Inhabitants by Province - The Netherlands - 2017", cex.main = .8)
```
...
```{r}
barplot(NL_Provinces2017_data$Total_population/1000000, names = NL_Provinces2017_data$Regional_level_label, las = 2, cex.axis = .6, cex.names = .6, ylab = "Inhabitants ( * 1,000,000)", col = NL_Provinces2017_data$Region_code, border = "grey", main = "Number of Inhabitants by Province - The Netherlands - 2017", cex.main = .8, legend.text = unique(NL_Provinces2017_data$Region_name), args.legend = list(x = 'topleft', bty = 'n', fill = unique(NL_Provinces2017_data$Region_name), border = 'grey'))
```
...
Create a pie chart:
```{r}
pie(NL_Regions2017_data$Total_population, labels = NL_Regions2017_data$Regional_level_label)
```
...
pct <- paste0(pct, "%")
lbls <- paste(NL_Regions2017_data$Regional_level_label, "\n", pct)
palette(c("royalblue3","firebrick3","darkolivegreen4","goldenrod1"))
pie(NL_Regions2017_data$Total_population, labels = lbls, clockwise = TRUE,
cex = .8, col = NL_Regions2017_data$Regional_level_label, border = "grey",
...
NL_Municipalities2017_data <- NL_Regional_Statistics2017 %>% filter(Regional_level %like% "GM")
