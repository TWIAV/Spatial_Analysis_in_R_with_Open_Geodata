...
The response contains also other valuable information like the maximum number of records returned per request:
```{r}
...
What is the number of records in the railroads dataframe? One would expect, no more than `r format(maxRecordCount, big.mark = ",")`.
...
A lot of the railroads are labelled 'abandonned'. They need to be filtered out. Of course you can do this *after* you fetched the records, but this isn't very efficient. Let's change the parameter value of `where` in our request, so only exploited rail roads are included in the response. 
...
`r format(nrow(exploited_railroads), big.mark = ",")` is still a lot of records, but let's have a look at the result and add the railroads on top of a map of the USA:
