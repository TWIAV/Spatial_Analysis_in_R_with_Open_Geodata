...
NL_Provinces2017_data %>% 
  select(Code, Name, Total_population, Region_code, Region_name) %>% st_set_geometry(NULL)
```

The `st_set_geometry(NULL)` removes the geometry in order to get some clean output.
...
````{r}
NL_Regions2017_data %>% select(Code, Name, Total_population) %>% st_set_geometry(NULL)
