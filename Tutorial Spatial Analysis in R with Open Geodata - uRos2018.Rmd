...
# Make sure to drop unused levels from the factors in those new data.frames (which are subsets of a larger data.frame)
...
We have noticed that the province names in the column `Name` all have the suffix *(PV)*. Let's get rid of it (but let's make sure it remains a factor):
...
As we have done in paragraph \ref{GFnl}
...
Conceptually, factors are variables in R which can only contain a pre-defined set of values, known as levels. Factors are often referred to as categorical variables, which can either be ordered (e.g. 'low', 'medium', 'high') or unordered (e.g. 'male', 'female').\par
One of the most important uses of factors is in statistical analysis and plotting. Storing categorical variables as factors insures that the statistical modeling functions will treat such data correctly. Factors are stored in R as a vector of integers, with a corresponding set of labels. These labels - one for each level - are used when the factor is displayed.\par
...
## Factor: group municipalities into categories

In this exercise we will divide our municipalities in 3 groups: small, medium and large.
...
```{r ordered_factor}
NL_Municipalities2017$Category[NL_Municipalities2017$Total_population < 50000] <- "Small"
NL_Municipalities2017$Category[NL_Municipalities2017$Total_population >= 50000 & NL_Municipalities2017$Total_population < 200000] <- "Medium"
NL_Municipalities2017$Category[NL_Municipalities2017$Total_population >= 200000] <- "Large"
```

```{r ordered_factor2}
class(NL_Municipalities2017$Category)

NL_Municipalities2017$Category <- factor(NL_Municipalities2017$Category, levels = c("Small","Medium","Large"), ordered = TRUE)
...
NL_Municipalities2017$Category[NL_Municipalities2017$Total_population < 20000] <- "Very small"
NL_Municipalities2017$Category <- factor(NL_Municipalities2017$Category, levels = c("Very small","Small","Medium","Large"), ordered = TRUE)
```



```{r tmapcategories}
qtm(shp = NL_Municipalities2017, title= "Municipalities by Size",
    fill = "Category", fill.title = "The Netherlands - 2017\nCategory", borders = "grey", format = "NLD_wide")
```

Please note: in the map printed above there are not 3 categories, but 4. We have added a category 'very small' for municipalities with less than 20,000 inhabitants.
