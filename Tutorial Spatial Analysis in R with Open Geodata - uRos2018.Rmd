...
However, in this case you need to change the units (and no need to divide by 10,000):
...
One of the nice things of sf objects is that you can take advantage of the dplyr functions like `select()`, `filter()`, `mutate()`, `group_by()` and `summarize()`. We already saw some examples in earlier chapters.
