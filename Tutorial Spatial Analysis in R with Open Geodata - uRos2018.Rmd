...
## How to access a WFS service? `request=GetCapabilities` \label{GCnl}
...
## Retrieving data from a WFS service: `request=GetFeature`\label{GFnl}
...
A WFS service can offer one or more feature collections. The `typenames` parameter determines the collection of feature instances to return. The service we are accessing here offers quite some feature collections, i.e. multiple regional divisions for the years 1995 up to the current year.

What we are intersted in here, is the municipal division for the year 2017. After some browsing through the XML response to our `GetCapalities` request, we have found a `FeatureType` with the `Name` **cbsgebiedsindelingen:cbs_gemeente_2017_gegeneraliseerd**. And that's the value we are giving to the `typename` parameter.
...
## Additional Exercise: an example from Finland
...
In this XML response we have found a `FeatureType` with the `Name` **vaestoalue:kunta_vaki2017**. That's the value we are giving to the `typename` parameter.
...
Optionally, you can have a look at the attribute data in the **Data Viewer**:

```{r eval = FALSE}
View(FI_Municipalities2018_Pop2017)
```
