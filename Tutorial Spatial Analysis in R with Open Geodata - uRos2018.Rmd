...
\href{https://www.linkedin.com/in/ejhpolle/}{Egge-Jan Pollé} - Tensing GIS Consultancy B.V.\\
\href{https://www.linkedin.com/in/willytadema/}{Willy Tadema} - Provincie Groningen  

Version 0.7.6 - September 3, 2018
...
`cbsodataR`, `data.table`, `dplyr`, `httr`, `knitr`, `jsonlite`, `mapview`, `sf`, `stringr` and `tmap`.
...
The thematic map below (see Figure \ref{example_tmap}) shows the population density in France in 2016, with an additional inset for *Île-de-France*, the region around Paris, as almost 20 percent of the French population lives there.
...
Let's put it all together in another example and query the Dutch Adresses en Buildings Register (Basisregistratie Adressen en Gebouwen) for the adresses in Hoek van Holland, a seaside town not far from The Hague.
...
Notice that we added `cql_filter=bag:woonplaats='Hoek van Holland'` to the request to filter out only adresses in Hoek van Holland.

Now the number of hits (`r format(hits, big.mark = ",")`) is greater than the maximum record count per request. We have hit the server limit. So how do we get all adresses if a request will not return more than `r format(maxRecordCount, big.mark = ",")`? The answer is simple: create more requests. This is called paging.
...
Because we know the maximum record count and the number of hits, we can calculate the number of requests required to fetch all records in the query result. By increasing the `startIndex` for each subsequent request, we can eventually retrieve all adresses in Hoek van Holland.
...
All `r nrow(format(adresses, big.mark = ","))` adresses in Hoek van Holland are fetched! To wrap it up, let's visualize them in an interactive map.

```{r eval = FALSE}
library(tmap)

tmap_mode('view')
tm_shape(adresses) +  tm_dots(col = "black", scale = 0.1) + 
...
Let's give it a try. We'll query the [USA Rail Road](https://www.arcgis.com/home/item.html?id=d209f26edc86485a9c631311e50d9940) dataset in ESRI's Living Atlas of the World.     
...
tm_shape(USA_States_2017) + tm_borders("black") + tm_shape(railroads_in_use) +
  tm_symbols(col = "red", scale = 0.1, border.lwd = NA) + tm_legend(show = FALSE) 
```
...
districts <- st_read(url) %>% select(code = WIJKCODE, name = WIJKNAAM)

# Trees in The Hague
url <- "https://ckan.dataplatform.nl/dataset/dd3873f6-b2d0-42e8-94c7-f7b47dcb71f0/resource/7ac8ba4a-586e-43f2-b12e-014079c83f00/download/bomen-csv.zip"
zip_file <- tempfile(fileext = ".zip")
...
unlink(zip_file)
rm(url, zip_file)
trees <- read.csv("./Data/bomen.csv", quote = "", sep = ";") %>%
...
You should always check whether all your datasets are in the same CRS:

```{r}
st_crs(districts)
st_crs(trees)
st_crs(bat_flight_paths)
```

The datasets are in the same CRS, namely WGS84 also known as EPSG:4326. Let's transform the data to RD_New to avoid warning messages in R later on in our analysis. RD_New or EPSG:28992 is the projected coordinate system for The Netherlands.    

```{r}
districts <- st_transform(districts, 28992)
trees <- st_transform(trees, 28992)
...
Let's begin with a simple question: What is the largest district in The Hague?

```{r}
districts$area <- st_area(districts)
districts %>% filter(area == max(area)) %>% select(name, area) %>% st_set_geometry(NULL)
```

We don't want the area in square meters, but in square kilometers. That should be easy:

```{r}
districts %>% filter(area == max(area)) %>% 
              select(name, area) %>% 
              mutate(area = area / 10000) %>% 
              st_set_geometry(NULL)
```

This isn't right. The units are still square meters!     
The units are set according to the CRS. This is a good thing, because different CRSs use different units. Some of them use meters, others use feet.    
However, in this case you need to change the units (and there is no need to divide by 10,000):
...
That's better!    
Next question: What are the five shortest bat flight routes? Print the lengths in kilometers - with two numbers after the decimal point. 

```{r}
bat_flight_paths$length <- st_length(bat_flight_paths)
bat_flight_paths %>% mutate(length = round(units::set_units(length, km), 2)) %>% 
                     arrange(length) %>% 
                     st_set_geometry(NULL) %>% 
                     head(n = 5)
```
...
But you can also aggregate spatial data, for instance calculate the city border using the districts dataset:

```{r}
the_hague <- st_union(districts)

tmap_mode("plot")
tm_shape(the_hague) +
  tm_borders("red", lwd = 2, lty = "dashed") +
tm_shape(districts) +
...
Or calculate the total number of trees for each district:

```{r warning = FALSE}
districts$total_trees <- lengths(st_covers(districts, trees))
```
...
```{r}
nrow(trees)
sum(districts$total_trees)
```

We're short `r nrow(trees) - sum(districts$total_trees)` trees in the district dataset! Where did those trees go?

```{r message = FALSE}
trees_not_in_district <- trees[the_hague, op = st_disjoint]
nrow(trees_not_in_district)
``` 

`r nrow(trees_not_in_district)` trees are not in a district of The Hague. Let's make a plot:
...
tm_shape(trees_not_in_district) +
  tm_symbols(col = "red", scale = 0.05, border.lwd = NA) +
tm_layout(frame = FALSE) + 
tm_legend(show = FALSE)
```

To be on the safe side, we'll remove all the trees from our dataset that are outside the districts. These are the red trees in the plot.
...
Another example of applying a spatial filter: Where are the bat flight paths in the Zorgvliet district?

```{r message = FALSE, warning = FALSE}
zorgvliet <- filter(districts, name == "Zorgvliet")
...
That's kind of ugly, those flight paths running outside the district. Let's cut them on the district border. Also add the trees to the plot.
...
Note that the dataset only contains trees in public space. But still, the tree density may give us an idea of how 'green' a district is.   

What are the districts with the least and the most trees?

```{r}
districts$name[which.min(districts$total_trees)] %>% as.character()
districts$name[which.max(districts$total_trees)] %>% as.character()
```

`r districts$name[which.min(districts$total_trees)] %>% as.character()` is a district with no trees at all in public space. Let's find out where it is.

```{r message = FALSE}
no_trees <- filter(districts, total_trees == 0)
...
Now visualize the tree density for each district in The Hague.

```{r}
districts$tree_density <- 
  round(districts$total_trees / units::set_units(districts$area, km^2))
qtm(shp = districts, fill = "tree_density", 
fill.palette = "Greens",
